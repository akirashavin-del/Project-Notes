...
const githubRequest = async (path, options = {}, customToken = null) => {
  const token = customToken || process.env.GITHUB_TOKEN;
  if (!token) throw new Error('GitHub publishing is not configured. Add GITHUB_TOKEN to the server environment or provide a personal token.');
...
export async function publishProject({ name, description, isPrivate = true, files = [], owner = process.env.GITHUB_DEFAULT_OWNER, userGithubToken = null }) {
  ...
  if (configuredMatch) {
    ...
    repository = await githubRequest(`/repos/${encodeURIComponent(targetOwner)}/${encodeURIComponent(targetName)}`, { method: 'GET' }, userGithubToken);
  } else {
    ...
    repository = await githubRequest('/user/repos', { method: 'POST', body: JSON.stringify({ name, description: description || 'Created by Project Notebook', private: isPrivate, auto_init: true }) }, userGithubToken);
  ...
  for (const file of files) {
    ...
    let sha;
    try { sha = (await githubRequest(path, { method: 'GET' }, userGithubToken)).sha; } catch (error) { if (!error.message.includes('(404)') && !error.message.includes('Not Found')) throw error; }
